  ...
  const renderAppointmentCards = (item, index) => {
    ...
    return (
      <View
        style={[
          styles.exprimaryBg,
          {flexDirection: 'row', marginBottom: 10, paddingHorizontal: 10},
        ]}>
        <View style={{width: '20%'}}>
          <Text
            style={[styles.primaryText, {textAlign: 'center', paddingTop: 12}]}>
            {keys[0]}:00
          </Text>
        </View>
        <View style={{flex: 0.9, flexDirection: 'column'}}>
          <FlatList
            data={item[keys[0]]}
            renderItem={({item1}) => renderSingleAppointment(item)}
            keyExtractor={item1 => item.id}
          />
        </View>
      </View>
    );
    ...
    return (
      <View
        ...
        <View>
          ...
          <Text style={styles.primaryText}>
            {capitalizeFirstLetter(item.service)}
          </Text>
        </View>
        <View>
