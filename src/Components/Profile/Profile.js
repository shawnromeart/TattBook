...
import { ImageBackground, StyleSheet, Text, View, Image, TouchableOpacity,Linking,Alert,FlatList, ActivityIndicator, ScrollView ,Dimensions, Platform, Modal} from 'react-native'
import { DefaultStyles } from '../AppKaStyle/DefaultStyles'
import { DarkModeStyles } from '../AppKaStyle/DarkModeStyles'
import {assets4Dark,commonColors,formatAppointmentTime,theme4Default,userProfileLink} from '../config'
import Feather from 'react-native-vector-icons/Feather';
import ProfileDetailsModal from './ProfileDetailsModal';
import AddHoursModal from './AddHoursModal'
import {useSelector,useDispatch} from 'react-redux'
import {fetchGallery} from '../../Apis/Appointment'
import {fetch_user_details,updateUserBackgroundColor,updateUserCoverImage,updateUserFontColor,updateUserFontFamily,updateUserProfilePic} from '../../Apis/Profile'
...
export default function Profile() {
    ...
    const [backgroundColor,setBackgroundColor]  = useState(userDetails.page_backgroundColor)
    ...
    const [totalUploadingImage,setTotalUploadingImage,totalUploadingImageRef] = useStateRef(0); 
    const [totalUploadedImage,setTotalUploadedImage,totalUploadedImageRef] = useStateRef(0);
    const [totalFailedUploadingImage,setTotalFailedUploadingImage,totalFailedUploadingImageRef] =useStateRef(0);
    ...
    const fontFamilies = [
                            ...
                            {label:"Wastinger Display Free Personal",valueIOS:"Wastinger Display Free Personal"},
            ...
            if (response.didCancel) {
                ...
                // console.log(response.assets[0]);
                    ...
                    if(mode== CHANGE_USER_IMAGE_MODE)
                    {
                        ...
                        updateUserProfilePic(response.assets[0].base64,userId,updatUserImageCallback)
                    }
                    else if(mode == CHANGE_HEADER_IMAGE_MODE)
                    {
                        setHeaderImage(source)
                        setLoadingHeaderImage(true);
                        updateUserCoverImage(response.assets[0].base64,userId,updatHeaderImageCallback)
                    }
                    else if(mode  == UPLOAD_GALLERY_IMAGE)
                    {
                        ...
                        response.assets.map(item =>{
                            uploadImageToServer({ name: 'image', filename:item.fileName, type: item.type, data: item.base64 } ,userDetails.name,galleryUploadCallback)
                        })
    ...
    function galleryUploadCallback(response)
    {
        ...
        if((totalUploadedImageRef.current+totalFailedUploadingImageRef.current)>=totalUploadingImageRef.current)
        {
    ...
    const renderOpeningTimeCard=(day,start,end,id)=>{
        ...
        return(
            <View style={Internalstyles.OpeningTimeWrapper}>
                <Text style={[styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>{day}</Text>
                <Text style={[styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>{start}</Text>
                <Text style={[styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>-</Text>
                <Text style={[styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>{end}</Text>
                <TouchableOpacity onPress={()=>createTwoButtonAlert('Slots',()=>deleteSlot(id,deleteCallback))} style={Internalstyles.iconWrapper}>
                    <Feather name="trash-2" size={18} style={[styles.textLinkColor,{color:"white"}]} />
                </TouchableOpacity>
            </View>
        )
    }
    ...
    function renderGalleryImages(item){
        ...
        return(
            <View style={[Internalstyles.GalleryWrapper]}>
                            <TouchableOpacity onPress={()=>createTwoButtonAlert('Gallery',()=>deleteImageCall(item.id,deleteImageCallback))}>
                                <Image source={assets4Dark.profile.remove} style={{height:20,width:20,position:'relative',top:0,left:0}} />
                            </TouchableOpacity>
                            <ImageModal
                                resizeMode="contain"
        ...
        :(
            <>
                  ...
                  <SafeAreaView edges={['top','left','right']}>
                    <ScrollView style={[styles.primaryBg,{height:height,backgroundColor:backgroundColor}]}> 
                        <View style={[styles.primaryBg, Internalstyles.container,{backgroundColor:backgroundColor}]}>
                            ...
                            <View style={{position: 'absolute', right: 4, top: 6,}}>
                                {styles == DarkModeStyles?(
                                    <View style={{backgroundColor: themeColors.PrimaryText, padding:10,borderRadius:5}}>
                                        <Text onPress={() =>action4DefaultMode()} style={{color:themeColors.primaryColors}} >Light</Text>
                                    </View>
                                ):(
                                    <View style={{backgroundColor: themeColors.PrimaryText, padding:10,borderRadius:5}}>
                            ...
                            <View style={{position: 'absolute', left: 0, top: 6,marginHorizontal:10}}>
                                {styles == DarkModeStyles?(
                                    <View style={{backgroundColor: themeColors.PrimaryText, padding:10,borderRadius:5}}>
                                        <Text onPress={() =>setColorModalVisible(true)} style={{color:themeColors.primaryColors}} >Change Colors</Text>
                                    </View>
                                ):(
                                    <View style={{backgroundColor: themeColors.PrimaryText, padding:10,borderRadius:5}}>
                                        <Text onPress={() =>setColorModalVisible(true)} style={{color:themeColors.primaryColor}}>Change Colors</Text>
                                    </View>
                                )}
                            </View>
                            ...
                            (
                            <ImageBackground source={headerImage} style={Internalstyles.profileHeader}>

                            </ImageBackground>
                            )}
                            <View style={[Internalstyles.HeaderRightEditIcon,Internalstyles.iconWrapper]}>
                                <TouchableOpacity onPress={()=>openGallery(CHANGE_HEADER_IMAGE_MODE)}>
                                    <Feather name="edit" size={18} style={[styles.primaryText,{color:"white"}]} />
                                </TouchableOpacity>
                            </View>
                            <TouchableOpacity onPress={()=>openGallery(CHANGE_USER_IMAGE_MODE)} >
                                {loadingUserImage?(
                            ...
                            <View style={Internalstyles.ProfileIdWrapper}>
                                <Text style={[Internalstyles.ProfileId, styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>{userName}</Text>
                            ...
                            {modalVisible?(
                                <ProfileDetailsModal styles={styles} modalVisible={modalVisible} userDetails={userDetails} setModalVisible={setModalVisible} setChanges={setChanges} anyChanges={anyChanges} setName={setName} setAbout={setUserAbout}/>
                            ):(null)}

                            {userDetails.phone?(
                                <View style={[Internalstyles.userProfileLinkWrapper]}> 
                                    <Text style={[styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>{userDetails.phone}</Text> 
                                </View>
                            ):(null)}  

                            <View style={[Internalstyles.userProfileLinkWrapper]}>
                                <TouchableOpacity onPress={()=>rateUsRedirect(userDetails.link)}>
                                    <Text style={[styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>{userDetails.link}</Text>
                                </TouchableOpacity>
                            </View>
                                
                            <View style={Internalstyles.TabsWrapper}>
                                <View>
                                    <Text style={[styles.primaryText, Internalstyles.TabsText,{color:fontColor,fontFamily:fontFamily}]} onPress={() =>setActiveTab(1)}>About</Text>
                                    {activeTab == 1?(
                                        <View style={[Internalstyles.activeTabColor, styles.activeTab]}></View>
                                    ):(
                                ...
                                <View >
                                    <Text style={[styles.primaryText, Internalstyles.TabsText,{color:fontColor,fontFamily:fontFamily}]} onPress={() =>setActiveTab(2)}>Gallery</Text>
                                        {activeTab == 2?(
                                            <View style={[Internalstyles.activeTabColor, styles.activeTab]}></View>
                                        ):(
                                            <View style={[Internalstyles.InactiveTabColor]}></View>
                                ...
                                <View style={Internalstyles.TabsBodyWrapper}>
                                    {activeTab == 1?(
                                        ...
                                        <View>
                                            ...
                                            <View style={[Internalstyles.aboutIntroWrapper,{marginBottom:5}]}>
                                                <Text style={[{fontSize: 20,},styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>Address</Text>
                                                
                                                <View style={Internalstyles.iconWrapper}>
                                                    <Feather name="edit" size={18} style={[styles.primaryText,{color:"white"}]} onPress={() => setModalVisible(true)} />
                                                </View>
                                            </View> 
                                            {(userDetails.locality||userDetails.city||userDetails.state||userDetails.zipcode)?(
                                                <Text style={[styles.primaryText, Internalstyles.aboutText,{color:fontColor,fontFamily:fontFamily,paddingTop:0}]}>
                                                    {userDetails.locality},{userDetails.city},{userDetails.state},{userDetails.zipcode}
                                                </Text>     
                                            ):(null)}
                                        ...
                                        <View style={[Internalstyles.aboutIntroWrapper]}>
                                            <Text style={[styles.primaryText, Internalstyles.aboutText,{color:fontColor,fontFamily:fontFamily}]}>{userAbout}</Text>
                                            <View style={Internalstyles.iconWrapper}>
                                                <Feather name="edit" size={18} style={[styles.primaryText,{color:"white"}]} onPress={() => setModalVisible(true)} />
                                            </View>
                                        </View>
                                        <View style={[Internalstyles.HoursWrapper]}>
                                            <Text style={[Internalstyles.HoursText, styles.primaryText,{color:fontColor,fontFamily:fontFamily}]}>Hours</Text>
                                            <View>
                                        ...
                                        {slots && slots.map((key) => (
                                            renderOpeningTimeCard(key.day,key.start,key.end,key.id)
                                        ))}
                                    ...
                                    <>
                                        ...
                                        <View style={[Internalstyles.GalleryWrapper]}>
                                            <FlatList
                                            data={galleryImages} 
                                            renderItem={({item})=>renderGalleryImages(item)} 
                                        ...
                                        <View style={[Internalstyles.GalleryWrapper]}>
                                            {galleryLoader?(<ActivityIndicator style={Internalstyles.profile} />)
                                            :
                                            (<TouchableOpacity onPress={()=>{openGallery(UPLOAD_GALLERY_IMAGE)}} style={{width:'25%', height:'auto'}}>
                                                <Image source={assets4Dark.profile.addImage} style={Internalstyles.GalleryImg} />
                                            </TouchableOpacity>)}
                            ...
                            {modalVisible4Hours?(
                                <AddHoursModal styles={styles} modalVisible4Hours={modalVisible4Hours} setModalVisible4Hours={setModalVisible4Hours} setChanges={setChanges} anyChanges={anyChanges} userDetails={userDetails}/>
                            ):(null)}
                    ...
                    <LoaderModal isImageMode={imageMode} totalFailedUploadingImage={totalFailedUploadingImage} totalUploadingImage={totalUploadingImage} totalUploadedImage={totalUploadedImage}  setModalVisible={setLoaderModalVisible} showModal={showModal}/>
            ...
            >
                    ...
                    <View style={[styles.apmodalView,{paddingTop:30}]}>
                        <View style={{alignItems: 'flex-end'}}>
                            <Icon name="close" size={24}  style={styles.primaryText} onPress={() => setColorModalVisible(!colorModalVisible)} />
                        </View>
                        <Text style={[{fontSize: 20, textAlign: 'center', marginBottom:20},styles.primaryText]}>Choose {colorPickerMode=="background"?'Background Color':'Font Color'}</Text>
                        ...
                        {  colorPickerMode == "background"?(
                            ...
                            <View style={{height:550,flexDirection:"column"}}>
                                <View style={{height:300}}>
                                    <ColorPicker 
                                        color={fontColor||"#fff"}
                                        swatchesOnly={false}
                                ...
                                <View style={{marginVertical:20}}>
                                    ...
                                    {fontFamilies.map(item =>{
                                            ...
                                            {
                                                fontName = item.valueAndroid
                                            }

                                            return( 
                                                <Picker.Item label={item.label} value={fontName}  />
                                            )
                                        })}
                        ...
                        <TouchableOpacity 
                        style={{alignSelf: 'center', backgroundColor:commonColors.LinksAndIndicatorColor, paddingVertical:10, paddingHorizontal:24,marginTop:20   }} 
                        onPress={()=>{
                            if(!colorSaveLoader)
                            {
                                ...
                                {
                                    updateUserFontColor(userDetails.temp_id,fontColor,(response)=>{setColorSaveLoader(false);setColorModalVisible(false);})
                                    updateUserFontFamily(userDetails.temp_id,fontFamily,(response)=>{console.log(response," font")})
                                }else
                                {
                                    updateUserBackgroundColor(userDetails.temp_id,backgroundColor,(response)=>{setColorSaveLoader(false);setColorModalVisible(false);})
                                }
                            }
                            
                        }}
                        >
                            {colorSaveLoader?(<ActivityIndicator size="large" color={themeColors.primaryColor} />):(
                        ...
                        >
                            <Text style={{color: 'white'}}>Choose {colorPickerMode=="font"?'Background Color':'Font Color'}</Text>
                        </TouchableOpacity>    
